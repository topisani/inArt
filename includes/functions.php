...
function get_user_info($login) {
    ...
    if ($stmt->num_rows == 1) return compact('user_id', 'email', 'username', 'password', 'salt');
    return false;
}
/**
...
function user_avatar_path($user_id) {
    ...
    if (!file_exists($avatar)) $avatar = $default;
    return $avatar;
}
